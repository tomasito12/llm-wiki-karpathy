...
- **Related tags:** ai-engineering, inference, orchestration, runtime-architecture, tool-use
...
- Mixture-of-Experts (MoE) is an architecture that routes each input, token, or task through only a subset of specialized submodels or expert blocks instead of activating the full network every time.
- The main practical effect is conditional compute: models can keep a very large total parameter count while using far fewer active parameters per inference step.
